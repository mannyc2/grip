...
/**
 * Pending payment created (awaiting claim)
 *
 * Sent to: Contributor without wallet (if they have account)
 ...
 */
export async function notifyPendingPaymentCreated(params: {
  recipientId: string; // User ID (contributor must have account)
  funderName: string;
  amount: string; // BigInt serialized as string
  tokenAddress: string;
  claimUrl: string;
  claimExpiresAt: string;
  bountyId?: string; // Optional: for bounty payments
  bountyTitle?: string; // Optional: for bounty payments
}) {
  const amountFormatted = (Number(BigInt(params.amount)) / 1_000_000).toFixed(2);

  // Title/message vary by context (bounty vs direct payment)
  const title = params.bountyId
    ? `${params.funderName} approved your bounty - $${amountFormatted} pending!`
    : `${params.funderName} sent you $${amountFormatted} - create wallet to claim`;

  const message = params.bountyId
    ? `"${params.bountyTitle}" - Create a wallet to claim your payment`
    : 'Create a passkey wallet to receive your payment';
