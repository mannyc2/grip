...
import { getOrgPendingLiabilities, getOrgWalletAddress } from '@/db/queries/organizations';
...
/**
 * Check if user has sufficient balance for new payment
 *
 * Validates: wallet balance >= existing liabilities + new payment amount
 *
 * @param userId - User ID of funder
 * @param walletAddress - User's Tempo wallet address
 ...
 */
export async function checkUserBalance(params: {
  userId: string;
  ...
  const [balance, liabilities] = await Promise.all([
    ...
    getFunderPendingLiabilities(params.userId),
  ]);
...
/**
 * Check if organization has sufficient balance for new payment
 *
 * Validates: org wallet balance >= existing liabilities + new payment amount
 *
 * @param orgId - Organization ID
 * @param tokenAddress - Token being sent
 * @param newAmount - Amount of new payment (BigInt)
 * @returns { sufficient: boolean, balance: bigint, totalLiabilities: bigint, walletAddress: string }
 */
export async function checkOrgBalance(params: {
  orgId: string;
  tokenAddress: string;
  newAmount: bigint;
}): Promise<{
  ...
  const walletAddress = await getOrgWalletAddress(params.orgId);
  ...
  const [balance, liabilities] = await Promise.all([
    ...
    getOrgPendingLiabilities(params.orgId),
  ]);
...
export const checkSufficientBalance = checkUserBalance;
